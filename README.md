...
| 层级       | 包路径          | 核心职责                                           | 设计特点       |
| ---------- | --------------- | -------------------------------------------------- | -------------- |
| **控制层** | `controller`    | 提供 HTTP 接口，包括请求解析、参数验证、响应封装等 | 倾向 RPC 风格  |
...
可发起 HTTP 请求调用数据服务（`<data source>Controller`）或运行 ETL 流程（`EtlController`），或者注册并定时调度任务（`JobController`），部分示例如下：

- 查询指定条数数据（需定义数据库源）

```http
GET /db1/select-random?limit=10
```
...
```http
POST /etl/file/run
[{"location": "scripts/seeding/standard_data.json"}]
```

```http
POST /etl/file/run-batch
[{"location": "scripts/seeding/advanced_data.json"}]
```
...
4. **接口调用**：查看 `/swagger-ui/index.html`（默认）接口文档，发起请求调用 HTTP 接口
