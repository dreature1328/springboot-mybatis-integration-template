...
def create_response(standardEntity):
    return {
        "success": True,
        "code": "SUCCESS",
        "message": "操作成功",
...
def save_to_file(standardEntity, filename):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    output_path = os.path.join(current_dir, filename)

    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(standardEntity, f, ensure_ascii=False, indent=2)

    print(f"保存文件: {output_path}")
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"数据条目: {len(standardEntity['standardEntity'])}")
